...
y = np.sin(x * 9) + (x ** 2) * 2
y += np.random.randn(*y.shape) * noise_mag

x_test = np.linspace(-1, 1, n_samples)
y_test = np.sin(x_test * 9) + (x_test ** 2) * 2
y_test += np.random.randn(*y.shape) * noise_mag
...
test_layer = tf.placeholder(dtype=tf.float32, shape=(n_samples, 1), name="test_layer")
input_layer = tf.placeholder(dtype=tf.float32, shape=(n_samples, 1), name="input_layer")
output_layer = tf.placeholder(dtype=tf.float32, shape=(n_samples,1), name="output_layer")
with tf.name_scope("layer_1") as scope:
    w_hidden = tf.Variable(tf.random_normal([n_input_features, n_hidden]))
    b_hidden = tf.Variable(tf.random_normal([n_hidden]))
    a_hidden = tf.nn.relu(tf.add(tf.matmul(input_layer, w_hidden), b_hidden))

with tf.name_scope("output") as scope:
    w_output = tf.Variable(tf.random_normal([n_hidden, n_output_features]))
    b_output = tf.Variable(tf.random_normal([n_output_features]))
    a_output = tf.add(tf.matmul(a_hidden, w_output), b_output)
    
loss = tf.losses.mean_squared_error(output_layer, a_output)
#test_loss = tf.losses.mean_squared_error(test_layer, a_output)
...
for i in range(n_epochs):
    ...
    test_loss_ = np.mean((a_output_-y_test)**2)
    ...
    if i % 100==0:
        ...
        pl.subplot(1,2,1)
        pl.cla()
        pl.plot(x_test,y_test, "bo")
        pl.plot(x_test,a_output_, "r")
        pl.subplot(1,2,2)
        pl.cla()
        pl.semilogy(train_loss, "b")
        pl.semilogy(test_loss, "r")
